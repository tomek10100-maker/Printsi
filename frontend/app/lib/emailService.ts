...
export const EmailTemplates = {
  ...
  orderShipped: (buyerName: string, productTitle: string, sellerName: string) => {
    const body = `
      ...
      <p style="margin:0 0 24px;color:#475569;font-size:15px;line-height:1.6;">
        You can track the progress of your delivery directly in the chat or on the Furgonetka.pl website if a tracking number was provided.
      </p>
  ...
  trackingAddedBuyer: (buyerName: string, productTitle: string, trackingCode: string) => {
    const trackingUrl = `https://furgonetka.pl/zlokalizuj/${trackingCode}`;
    const body = `
      <p style="margin:0 0 12px;color:#1e293b;font-size:20px;font-weight:700;">Your package has a tracking number! 📦</p>
      <p style="margin:0 0 24px;color:#64748b;font-size:16px;line-height:1.6;">
        We've assigned a Furgonetka.pl tracking number to your order: <strong>${productTitle}</strong>.
      </p>
      ...
      <div style="background:linear-gradient(135deg,#fffbeb,#fef3c7);border-radius:16px;padding:28px;margin:24px 0;border:1px solid #fcd34d;text-align:center;">
        ...
        <div style="color:#92400e;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px;">Furgonetka.pl Tracking Number</div>
        <div style="color:#d97706;font-size:28px;font-weight:900;font-family:monospace;letter-spacing:2px;">${trackingCode}</div>
      </div>

      <div style="text-align:center;margin:28px 0;">
        <a href="${trackingUrl}" style="display:inline-block;background-color:#ffcc00;color:#1a1a1a;text-decoration:none;font-size:16px;font-weight:900;padding:16px 32px;border-radius:12px;box-shadow:0 4px 6px rgba(0,0,0,0.15);">
          🔍 Track on Furgonetka.pl
        </a>
      </div>

      <p style="margin:0;color:#94a3b8;font-size:13px;line-height:1.5;text-align:center;">
        Tracking may take a few hours to become active on the Furgonetka.pl website after the package is handed to the courier.
      </p>
    `;
    return emailWrapper(
  ...
  trackingAddedSeller: (sellerName: string, productTitle: string, trackingCode: string) => {
    const body = `
      ...
      <p style="margin:0 0 24px;color:#64748b;font-size:16px;line-height:1.6;">
        A Furgonetka.pl tracking number has been added to the order: <strong>${productTitle}</strong>. The buyer has been notified.
      </p>
      ...
      <p style="margin:0 0 24px;color:#475569;font-size:15px;line-height:1.6;">
        The buyer can now track the package through Furgonetka.pl directly from their chat. Once they confirm delivery, funds will be released to your balance.
      </p>
