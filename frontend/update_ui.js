...
// 1. Add close modal on chat change
txt = txt.replace(
    /(const messagesEndRef = useRef<HTMLDivElement>\(null\);)/,
    `$1\n    useEffect(() => {\n        setShowProposalModal(false);\n    }, [activeChatId]);`
);

// 2. Wrap handleRejectProposal and handleRenegotiate
const renegotiateLogic = `
    ...
    const handleRenegotiate = (msgId: string, parsedData: any) => {
        handleRejectProposal(msgId, parsedData);
        setProposalPrice(parsedData.price ? String(parsedData.price) : '');
        setProposalQty(parsedData.quantity ? String(parsedData.quantity) : '1');
        setProposalMaterial(parsedData.material === 'Any' ? '' : parsedData.material);
        setProposalColor(parsedData.color === 'Any' ? '' : parsedData.color);
        setProposalColorHex(parsedData.colorHex || '#cccccc');

        if (parsedData.dimensions) {
            setProposalDims(parseDimensionsAdvanced(parsedData.dimensions));
        } else {
            setProposalDims([]);
        }
...
txt = txt.replace(/const handleRejectProposal = [\s\S]*?loadMessages\(activeChatId as string\);\s*};/, renegotiateLogic);

// 3. Update the modal message rendering
...
txt = txt.replace(msgRenderOriginal, msgRenderNew);

// 4. Update the Buttons
const buttonsOriginal = `                                                            {/* ACTIONS */}
...
txt = txt.replace(buttonsOriginal, buttonsNew);
